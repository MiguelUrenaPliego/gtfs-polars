...
class GTFS(object):
    ...
    def __init__(self, gtfs_dir:str|list[str], service_date:str, 
                 start_time:str='00:00:00',end_time:str='00:00:00', 
                 bounds:gpd.GeoSeries|gpd.GeoDataFrame=None, strict_bounds:bool=True,
                 stop_group_distance=0,trip_group_distance=0,trip_group_overlap=0.75,
                 correct_stop_sequence=True,crs=4326, route_filter:pd.DataFrame|pl.DataFrame=None):
        """
    ...
    def __read_stop_times(self,trips=None,reload_data:bool=False):
        ...
        if type(stop_times) == type(None):
            ...
            ).filter(pl.col('departure_time').is_null() == False)
        ...
        if type(shapes) == type(None):
            ...
            if len(files) > 0:
                shapes = pl.concat([self.__read_csv(os.path.join(self.gtfs_dir[j], 'shapes.txt'), 
                                schema_overrides={'shape_id': str,'shape_pt_lat':float,'shape_pt_lon':float,'shape_pt_sequence':int}
            ...
            shapes = gpd.GeoDataFrame(shapes['shape_id'].to_pandas(),geometry=gpd.GeoSeries.from_wkt(shapes['wkt'].to_pandas()),crs=self.crs)
    ...
    def __get_service_ids(self,calendar,calendar_dates,service_date,counts_by_date=None):
        ...
        if type(service_date) != datetime:
            ...
            service_date = datetime(int(service_date[0:4]), int(
                            service_date[4:6]), int(service_date[6:8]))
    ...
    def stop_quality_class(self,class_frequencies,start_time=None,end_time=None,agg='sum',agg_factor:float=1,exclude_first_stop:bool=True):
        ...
        capacity = capacity.with_columns(
            ...
            service_class = len(class_frequencies)+1
        )
        service_class = len(class_frequencies)
        for c in np.sort(class_frequencies)[::-1]: 
            capacity = capacity.with_columns(
                service_class = pl.when(pl.col('frequency') < c).then(service_class).otherwise(pl.col('service_class')).cast(int)
            )
            service_class -= 1 
